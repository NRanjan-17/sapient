...
- [x] **Sample apps** (2026-07-11) — three chat apps in `examples/`, all
  ...
  `swift build`, iOS app generated by XcodeGen — the committed `project.yml`;
  ...
  xcodegen ≥2.44 stamps the Xcode 16 project format [objectVersion 77] that
  Xcode 15 refuses to open — `project.yml`'s `postGenCommand` downgrades it),
  `android-chat` (Compose; consumes the packaged module via
  `include(":sapient-android")`; **caught a real FFI bug** — error-enum
