...
static inline float f16_at_o(const uint8_t *p) {
    uint16_t bits; memcpy(&bits, p, sizeof bits);
    __fp16 h; memcpy(&h, &bits, sizeof h);
    return (float)h;
}

static inline void gsm(size_t j, const uint8_t *scales, uint8_t *sc, uint8_t *m) {
    ...
    const int32_t *x_sums)
{
    ...
    for (size_t b = 0; b < nlim; ++b) {
        ...
        for (size_t r = 0; r < 4; ++r) {
            dv[r]    = f16_at_o(rp[r]);
            dminv[r] = f16_at_o(rp[r] + 2);
            const uint8_t *sp = rp[r] + 4;
            for (size_t j = 0; j < 8; ++j) gsm(j, sp, &SC[r][j], &MN[r][j]);
        }

        // (2) vector accumulators — one horizontal reduction per row per block.
        int32x4_t vs0 = zero, vs1 = zero, vs2 = zero, vs3 = zero;
        ...
        for (size_t g = 0; g < SAPIENT_QK_K / 64; ++g) {
            ...
            do {                                                                     \
                ...
                VACC = vmlaq_n_s32(VACC, dhi, (int32_t)SC[R][is + 1]);               \
                imin[R] += (int32_t)MN[R][is] * sum_lo                               \
                         + (int32_t)MN[R][is + 1] * sum_hi;                          \
            } while (0)

            ROW(0, vs0); ROW(1, vs1); ROW(2, vs2); ROW(3, vs3);
            #undef ROW

            x_off += 64; q_off += 32; is += 2;
        }

        const int32_t isum[4] = {
            vaddvq_s32(vs0), vaddvq_s32(vs1), vaddvq_s32(vs2), vaddvq_s32(vs3)
        };
        for (size_t r = 0; r < 4; ++r) {
